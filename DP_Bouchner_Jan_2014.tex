...
Závěrečnou kapitolu bych rád věnoval zhodnocení celé své práce. Zaměřím se také na další možné kroky v budoucím vývoji aplikace. 

Na základě definovaných požadavků a znalostí nabytých vlastním výzkumem byl vyvinut adaptibilní systém pro doporučování obsahu a několik součástí nezbytných pro vzájemnou komunikaci s tímto systémem a okolními komponentami. 
...
Při tvorbě součástí systému bylo využito několik moderních technologií, jejichž snahou je poskytovat uživateli rychlý a distribuovaný přístup a stavět robustní a odolná řešení. 

Kvůli potřebám snadné komunikace a provádění experimentů se systémem jsem byl nucen vyvinout testovacího klienta~\ref{chap:client} schopného komunikovat se všemi komponentami vystavěného doporučovacího prostředí. Do tohoto prostředí spadá fulltextový index uchovávající informace o článcích a uživatelských hodnoceních a stejně tak i doporučovací algoritmy přístupné díky HTTP metodám RESTful API.

Výsledné experimenty se systémem jsem bohužel nestihl provést na žádném standardním datasetu a zůstal tak odkázán na vlastní jednoduchý generátor, jímž jsem vytvářel \uv{články} a \uv{uživatelská hodnocení} v Apache Solr již v prvních týdnech implementace. Testovací klient využíval těchto informací z indexu po celou dobu experimentů. To samozřejmě přineslo některé nepříjemnosti v závěru, například to, že uživatelská hodnocení položek nebyla dostatečně vypovídající například pro potřeby kolaborativního filtrování. Dalo by se říci, že tato hodnocení jsou jen náhodným shlukem položek u článku.

Další rozvoj systému je nicméně v plánu a ani zdaleka se nejedná o uzavřenou věc.
