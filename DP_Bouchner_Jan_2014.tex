...
Většinová funkcionalita je řešena třídou \texttt{MultiThreadServer} (obsahující též vnitřní třídu  \texttt{WorkerThread}) z balíčku projektu Ensemble:
...
Vláknům je parametrem předána instance třídy \texttt{RequestHandlerJson}, takže jsou schopny obsluhovat příchozí žádosti ve formátu JSON. Jedná se o implementaci rozhraní \texttt{RequestHandler} s jednou definovanou metodou pro obsluhu zpráv. Tento typ přijímají ve svém konstruktoru i pracovní vlákna. Řešení je tedy postaveno tak, aby bylo snadno rozšiřitelné pro jakýkoliv jiný formát komunikace, klidně vlastní.
...
O několik řádku výše bylo vysvětleno, jakým způsobem jsou na serveru zpracovány příchozí žádosti. Nyní již víme, že tuto funkcionalitu mají na starosti pracovní vlákna typu \texttt{WorkerThread}.
...
Třída \texttt{WorkerThread} je vláknová; kromě konstruktoru obsahuje pouze jednu metodu \texttt{run()}. Právě tato metoda má na svědomí veškerou obsluhu žádosti zahrnující zpracování zprávy, volbu správné operace na rozhraní systému a po zpracování pak zaslání odpovědi zpátky k žádajícímu klientovi.
...
Výsledkem zpracování zprávy je tedy operace realizována návrhovým vzorem Command. \texttt{Operation} je rozhraní deklarující metody pro validaci operace (\texttt{validateOperation()}) a její vykonání (\texttt{executeOperation()}). Každá třída implementující toto rozhraní je pak konkrétní operací, která volá rozhraní systému Recommeng. 
...
Výsledkem vykonání operace je vždy odpověď (třída \texttt{Reply} s atributy pro stavový kód a tělo zprávy), která je pomocí třídy \texttt{ResponseHandlerDefault} prezentována zpět klientovi.
